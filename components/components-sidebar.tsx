...
// Animated category item with dropdown
...
function AppSidebar({ currentPath }: { currentPath: string }) {
  // Find which category contains the current path and open it by default
  const activeCategory = categories.find((cat) =>
    cat.components.some((comp) => comp.href === currentPath)
  );

  const [openCategories, setOpenCategories] = useState<string[]>(
    activeCategory ? [activeCategory.name] : ["Shadcn+"]
  );
...
function ComponentsHeader({ title }: { title?: string }) {
  return (
